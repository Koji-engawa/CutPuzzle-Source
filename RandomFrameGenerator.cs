...
//ランダムな形状のフレームを生成する
public class RandomFrameGenerator : MonoBehaviour
{
    private Material FrameMaterial;

    public int maxVertices = 25;
    ...
    void Awake()
    {
        FrameMaterial = Resources.Load<Material>("FrameMaterial");
    ...
    Mesh GenerateRandomMesh()
    {
        ...
        int vertexCount = UnityEngine.Random.Range(10, maxVertices + 1);
    ...
    //生成したメッシュを使用してフレームを生成する
    void DisplayMesh(Mesh mesh)
    {
        MeshFilter meshFilter = gameObject.AddComponent<MeshFilter>();
        MeshRenderer meshRenderer = gameObject.AddComponent<MeshRenderer>();
        meshFilter.mesh = mesh;
        meshRenderer.material = FrameMaterial;
        ...
        edgeCollider.points = mesh.vertices.Select(v => new Vector2(v.x, v.y)).ToArray();
        edgeCollider.isTrigger = true;

        gameObject.AddComponent<ClearChecker>();
    }
}